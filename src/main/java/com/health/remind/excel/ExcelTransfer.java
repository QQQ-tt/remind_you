...
public class ExcelTransfer {
    ...
    public static <I> void exportExcel(HttpServletResponse response, List<I> list, String name, String sheet,
        ...
        exportHeader(response, list, name, sheet, headerList);
    }

    private static <T> void exportHeader(HttpServletResponse response, List<T> list, String name, String sheet,
                                         List<String> headerList) {
        try {
            response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
            response.setCharacterEncoding("utf-8");
     ...
     * @param list     实体:数据
     */
    public static void exportExcel(HttpServletResponse response, String name, List<ExcelEntity> list) throws IOException {
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setCharacterEncoding("utf-8");
        // 这里URLEncoder.encode可以防止中文乱码 当然和easyexcel没有关系
        String fileName = URLEncoder.encode(name, StandardCharsets.UTF_8)
    ...
    public static void exportExcel(HttpServletResponse response, String name, ExcelEntity excelVO) throws IOException {
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setCharacterEncoding("utf-8");
        // 这里URLEncoder.encode可以防止中文乱码 当然和easyexcel没有关系
        String fileName = URLEncoder.encode(name, StandardCharsets.UTF_8)
